...
#
# 2026-08-21: 둘을 다시 벌린다. 08-14 에 같은 값으로 둔 이유는 "가용
# 범위가 0.8~1.1N 뿐이라 벌려 봐야 차이가 F_DEADBAND 보다 작다"였는데,
# 08-18/19 로그 30회를 다시 재 보니 그 전제가 틀렸다:
#   HOLD 중 힘 중앙값 1.06N, p90 1.91N (최대 2.06N)
# 즉 실제 가용 범위는 0.8~1.1 이 아니라 대략 0.3~2.0N 이다. 1.2 와 1.8
# 의 차이 0.6N 은 F_DEADBAND(0.3N)의 두 배라 실제로 구분된다.
#
# 같은 값으로 두면 분류가 파지력을 전혀 안 바꾼다 -- 로그 67건의
# object_class 가 전부 "soft" 였던 것과 합쳐져서, 강성 추정 전체가
# 로그 말고는 아무 데도 안 쓰이는 상태였다.
#
...
#
# SOFT 를 그때 실패한 0.8 이 아니라 1.2 로 잡는 이유: 힘 중앙값 1.06N
# 위쪽이라 "접촉만 해도 목표를 넘어 손가락을 펴는" 경로에 안 들어간다.
# 게다가 그때는 없던 HOLD_OPEN_LIMIT 이 지금은 여는 폭을 막아 준다.
F_TARGET_SOFT = 1.2    # 연체로 판정 -- 힘 중앙값(1.06N) 바로 위
F_TARGET_RIGID = 1.8   # 강체로 판정 -- 실측 p90(1.91N) 근처
...
# 정했다"가 드러나는 편이 낫다.
#
# 2026-08-21: 값을 정한다. 08-18/19 로그 25개 런의 대표 k_hat(손가락별
# 최대, confident 만)을 정렬하면 틈이 딱 한 군데 있다:
#
#   1.38 1.98 2.33 2.42 2.81 2.87 2.91 3.19 3.74 4.36 4.51 5.08 5.09
#   5.15 5.76 6.71 7.22 9.67 10.42 10.49 10.88 14.72 15.15
#                        <-- 12.4배 틈 -->
#   187.29  208.56
#
# 1.38~15.15 에 23개가 연속으로 몰려 있고, 그 위 두 개만 두 자릿수
# 배율로 떨어져 있다. 정렬값 사이에서 1.6배 이상 벌어진 곳이 여기
# 하나뿐이다. 두 값 다 confident=True 라 측정 실패 sentinel(K_MAX)이
# 아니라 실제 회귀 결과다. 기하중간 √(15.15 x 187.29) ≈ 53 -> 50.0.
#
# --- 이 값을 얼마나 믿을 수 있나 ---
# 위쪽 표본이 2개뿐이라 약하다. 그리고 각 런이 어떤 물체였는지가
# 로그에 없어서, 저 둘이 정말 강체였는지는 확인되지 않았다. 그래도
# None 으로 두는 것보다는 낫다 -- None 은 k_hat 을 보지도 않고 전부
# soft 로 보내서 분류 자체를 무효로 만든다. 잠정값으로 두고 로그의
# class 열로 계속 검증한다.
#
# 제대로 정하려면 스펀지/종이컵/나무토막을 각각 잡은 k_hat 이 필요하다.
K_THRESHOLD = 50.0
...
# --- 벌림 탐색 ---
# 안정화 시점에 접촉한 손가락이 이 수보다 적으면, 못 찾은 손가락을
# 옆으로 훑어 파지 자리를 찾아본다(spread_seek 참고).
#
# 3 인 이유: 08-18/19 로그 30회의 안정 HOLD 손가락 수 분포가
#   0개 5회 / 1개 6회 / 2개 13회 / 3개 3회 / 4개 3회
# 로 중앙값이 2다. 기준을 2로 두면 절반이 그냥 통과해서 탐색이 거의
# 안 돌고, 4로 두면 거의 매번 돌아 파지마다 몇 초를 더 쓴다. 3은
# "지금보다 낫게"와 "매번 훑지는 않게" 사이다. 미검증 시작값이다.
SEEK_MIN_CONTACT = 3

# 탐색을 켤지. 실물에서 손가락이 물체를 밀어내는지 먼저 봐야 하므로
# 스위치를 남긴다.
SEEK_ENABLED = True
